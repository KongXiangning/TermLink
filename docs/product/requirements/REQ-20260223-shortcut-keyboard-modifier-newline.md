...
related_code: [public/terminal_client.js, public/terminal_client.css, public/terminal_client.html, android/app/src/main/java/com/termlink/app/MainShellActivity.kt, android/app/src/main/res/layout/activity_main_shell.xml]
...
3. 新增“换行”按钮并发送 `\r`（Enter）输入。
4. 快捷键盘按钮态需可视化区分（普通态/按下态/一次性锁定态）。
...
1. 在 `terminal_client.js` 引入快捷键盘控制键状态机，支持 `Ctrl/Alt` 一次性组合发送。
2. 为“换行”新增按钮事件处理，直接透传 Enter 到终端输入通道。
...
1. 单键验证：普通字母输入保持不变。
2. 组合键验证：`Ctrl+C`、`Ctrl+L`、`Alt+F`、`Alt+B`。
3. 换行验证：输入命令后点击“换行”，观察命令执行。
...
3. 缓解：限制状态只在下一次按键消费，并在发送后立即复位；滑动调整需做真机回归。
