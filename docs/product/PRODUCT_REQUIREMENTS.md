...
16. Relay 控制平面与透明中转模式（`REQ-20260413-relay-control-plane-and-transparent-transit`，`planned`）：为无法暴露公网的后端服务规划控制台、connector 与透明 Relay 主链路；首期固定为“管理员预创建 `app_client` 槽位 + 一次性注册码认领”的配对模型，采用单组织 + 单管理员操作面，数据面只透传既有 `sessions list/create/delete/rename API + terminal WebSocket`，继续保持 `relay ingress` 与 `target identity` 分离、service 只能发布 connector inventory 中已确认的目标；同时明确首批实现会强关联当前 TermLink 后台与 Android App，但 Relay 控制面 / Transit / Connector 需按未来可抽离独立项目的边界、稳定标识和显式失效治理设计。
17. Codex Android 运行态一致性、通知与关键交互修复（`REQ-20260415-codex-android-runtime-interaction-fixes`，`in_progress`）：三批代码修复、Android 单元测试 / debug 构建与 Huawei 真机闭环已全部完成，已覆盖终止按钮误报、idle 状态下“任务进行中”通知残留、任务完成提醒缺失、顶部 header 对前摄下方空间利用不足、“返回最新”按钮自动隐藏失效、底部 `/` 不写入 composer、历史线程继续发送误建新任务，以及弱网后结果无法从历史完成态回补；当前仅待关联 CR 的真实 `commit_ref` 回填后做最终关单。
