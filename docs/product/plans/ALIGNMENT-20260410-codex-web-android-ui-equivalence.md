...
### 9.5 2026-04-13 stability / plan UX / runtime readability 开放差异

以下 10 项差异来自 2026-04-13 新一轮真机使用反馈，当前均应视为开放问题，不能继续按“已完成收口”处理。

| id | 界面单元 | Web / 目标口径 | Android 当前行为 | 阻塞上线 | 收敛方式 | 验证方式 |
| --- | --- | --- | --- | --- | --- | --- |
| D-018 | 后台恢复 / 连接稳定性 | 原生 Codex 切后台切回后应继续保持单一、可恢复的连接状态；若断联，也应先收敛为统一 reconnect/retry 路径，仅在最终失败时给出单条用户可理解的失败反馈，而不是直接向聊天流抛出多条底层 socket 错误 | 真机切后台切回后出现 `Broken pipe`、`WebSocket failure`、`Software caused connection abort` 等错误卡片，且会直接打断当前阅读与操作 | 是 | 复核 `CodexActivity` 生命周期、前台服务 owner、socket 重连与错误去重策略，统一后台恢复失败的 UI 口径，并把底层错误与聊天流展示解耦 | 真机 `HOME -> foreground`、近期任务切换、锁屏返回回归 + logcat |
| D-019 | Plan workflow / composer | 计划正文应有单一主落点，默认以运行态 `Plan` 为准；composer 上方只保留紧凑动作条承载 `执行 / 继续 / 取消`，不在聊天流和输入框上方重复刷完整计划 | 当前计划会同时出现在聊天消息、输入框上方 workflow 区和运行态 `Plan`，阅读负担明显偏高 | 否 | 重构 plan workflow 信息架构：正文下沉到 runtime，composer 仅保留状态摘要和显式动作入口，明确“正文展示”和“动作入口”分离 | Web/Android 对照 + 真机计划模式回归 |
| D-020 | 计划执行确认 / 聊天流 | 确认执行计划后，聊天流不应再次完整回显同一份计划正文；应仅保留执行确认语义或状态摘要，避免重复展示 | 当前选择执行后，对话中还会再出现一遍完整计划，造成重复刷屏 | 否 | 让执行动作复用已确认计划引用或摘要，不再回灌完整正文消息，计划正文继续只在 runtime `Plan` 查看 | 真机 `plan_ready_for_confirmation -> execute` 回归 |
| D-021 | Runtime / Diff | Runtime `Diff` 首屏应先让用户看清“哪些文件变了”，采用摘要优先，先展示文件级摘要、分组标题或文件列表，再展开具体 diff 文本 | 当前 `Diff` 信息杂乱，用户无法一眼识别被修改文件 | 否 | 为 diff 增加文件级摘要、分组标题或文件列表，再展示 unified diff 正文 | 真机 runtime 截图 + 真实改文件样本回归 |
| D-022 | Runtime / Reasoning | 若上游有高价值 reasoning，则应以可读摘要呈现；若没有稳定高价值内容，则前端应明确降级为空态解释或隐藏，而不是保留噪音区块 | 当前 `Reasoning` 没有给出用户可消费的有效信息 | 否 | 明确 reasoning 的展示准入与降级策略，必要时改为空态解释、摘要或隐藏条件 | 真机带 reasoning/无 reasoning 两类样本对照 |
| D-023 | Image attachments | 图片加入待发送区后应形成真实上传/发送闭环，而不是仅在本地 UI 里出现 chip | 当前图片添加后并未形成真实上传 | 是 | 复核 Android 图片附件编码、请求载荷、服务端消费与发送成功反馈链路 | 本地图片 + URL 图片真机回归 |
| D-024 | Approval / escalation entry | 当命令或文件操作需要用户批准时，原生安卓 Codex 页面应提供与桌面端一致的明确提权/批准入口，并以阻塞弹窗承载批准流程，而不是放在运行态入口或隐藏入口 | 当前需要提权时，原生安卓 Codex 页面没有出现提权选项，用户无法完成批准 | 是 | 复核 pending approval/request_user_input 在 Android 端的展示入口、触发条件与弹窗承载方式，补齐可操作的批准 UI | 真机构造工作区外写入或需提权命令场景回归 |
| D-025 | Header / thread metadata | 顶部 header 应保持稳定，不应因长 thread/task id 进入而被挤压变形；线程标识应摘要化或放到次级区域 | 选择打开任务后，顶部状态栏直接显示长 ID，导致 header 布局变化 | 否 | 将 thread id 改为截断摘要、短标签或移入次级信息区，避免主 header 横向跳动 | 真机打开历史任务 / 新线程切换回归 |
| D-026 | Runtime / stuck-state detection | 主窗口长时间无输出、未报错、未结束时，不应无限停留在模糊“运行中”状态；应先进入“疑似卡住”告警，并在可识别时给出原因分类，而不是要求实现阶段直接 100% 判定服务端已失败 | 当前任务有时会长时间卡住，无报错、无结束、无明确状态，只能手动中断 | 是 | 补齐运行态心跳、最近事件时间、状态诊断展示与重试入口；将“疑似卡住”与“已失败”分层处理，并在可识别时分类为等待用户输入/提权、事件流停滞、长时间无新输出、连接已断或任务已失败 | 真机构造长时间无输出但仍 running、等待用户输入/提权、真实断连或任务失败等样本回归 |
| D-027 | Chat transcript / speaker distinction | 主聊天窗口中的用户消息与助手消息应采用左右分栏 + 明显样式差异，且不仅区分颜色，还要区分对齐、容器样式和标签层级，保证长对话回看时能快速定位发言方 | 当前主聊天窗口中用户与助手消息区分不明显，回看和查找自己发起的消息成本高 | 否 | 统一用户/助手消息的对齐、容器、标签和层级策略，避免错误卡片、运行态信息和长回复混排时继续混在一起 | 长对话、多轮追问、助手长回复、错误卡片混排场景下真机回归 |
...
### 10.7 2026-04-13 stability / plan UX / runtime readability 文档初始化产出

1. 已在 `PLAN` 中新增一批 `pending` follow-up，锁定后台恢复报错、计划模式重复展示、执行后计划重复回显、runtime diff/reasoning 可读性、图片真实上传与 header thread id 布局抖动问题。
2. 已在本文新增 `D-018 ~ D-025` 八条开放差异，明确这些问题当前仍未收敛，且其中后台恢复、图片真实上传与提权入口缺失属于阻塞性缺口。
3. 已创建并回写 [CR-20260413-0832-codex-android-followup-doc-init.md](/E:/coding/TermLink/docs/changes/records/CR-20260413-0832-codex-android-followup-doc-init.md) 追踪本批文档初始化范围。
