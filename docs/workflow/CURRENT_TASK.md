...
- Finding ID：RF-20260619-002
  ...
  - Status：resolved
  - File / symbol：`public/terminal_client.js` `handleCodexIpcConversations` / `selectCodexIpcConversation`
  ...
  - Ownership assessment：current_task_owned，属于本轮 approval / IPC routing 改动引入的回归；根因确认落在当前 Allowed Files 内的 IPC conversation selection。
  ...
  - Correction：2026-06-19 用户更正“实时同步依然失效”为“会话自动加载回另一个任务；另一个任务可以实时同步”。调查重点从 feed 断流收敛为 conversation ownership / active selection / replay gating。
  - Recommended route：current_task_owned / queued_fixable_findings
  - Recommended handoff：implement-current-step
  - Resolution：2026-06-19 已调整 `selectCodexIpcConversation()`：只允许当前 `threadId` 精确匹配主动切换；已有 `activeConversationId` 仍存在时保持；`lastCodexThreadId` 仅保留为 Codex resume/bootstrap 线索，不再直接驱动 IPC `set_active_conversation`，避免 stale last thread 把页面切回另一个可实时同步的旧任务。
  - Resolution evidence：`node --check public/terminal_client.js` 通过；`node --test --test-name-pattern "conversation selection|active conversation|lastCodexThreadId|set_active_conversation" tests/codexSecondaryPanel.integration.test.js tests/codexClient.shell.test.js` 9/9 pass。
...
- 2026-06-19：继续修复 RF-20260619-002。根因收敛为 `selectCodexIpcConversation()` 在 conversation list 更新时允许 `lastCodexThreadId` 直接命中并发送 `set_active_conversation`；当该值指向另一个旧任务时，旧任务 snapshot 会被 replay 到当前页面，表现为“打开任务后自动加载回另一个任务”，且旧任务本身实时同步正常。修复：`lastCodexThreadId` 不再作为 IPC auto-select 输入；选择顺序改为当前 `threadId` 精确匹配、保持现有 active conversation、无 thread/last 线索且仅一个 conversation 时安全订阅。同步补强 IPC approval 前端点击路径：IPC request 使用 `decision` response mode，approval 提交测试直接验证 `submitBlockingCommandApprovalDecision()` 会发送 `follower_approval_response`，失败发送不会把 request 标成 submitted。验证：`node --check public/terminal_client.js` 通过；`node --test --test-name-pattern "conversation selection|active conversation|lastCodexThreadId|set_active_conversation" tests/codexSecondaryPanel.integration.test.js tests/codexClient.shell.test.js` 9/9 pass；`node --test --test-name-pattern "IPC approval request|failed IPC approval send|submitBlockingCommandApprovalDecision" tests/codexSecondaryPanel.integration.test.js tests/codexClient.shell.test.js` 3/3 pass；`node --test --test-name-pattern "approval projection|gateway resolves|conversation selection|submitBlockingCommandApprovalDecision|follower_send_message and set_active_conversation" tests/codexClient.shell.test.js` 5/5 pass；`node --check src/services/codexIpcThreadStream.js`、`node --check src/ws/terminalGateway.js`、`node --check public/lib/codex_approval_view.js` 通过；`node --test tests/codexApprovalView.test.js tests/codexIpcThreadStream.test.js` 32/32 pass。整文件 `tests/codexClient.shell.test.js` 仍有既有 HTML 壳断言失败（缺 `btn-codex-secondary-threads`），与本轮 IPC selection/approval 修复无关；`tests/terminalGateway.codexIpc.test.js` 的 follower approval targeted run 仍在 90 秒超时，延续该文件既有 open-handle 风险，本次未修改 gateway。
