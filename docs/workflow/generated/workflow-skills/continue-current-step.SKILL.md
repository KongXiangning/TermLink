...
must_check:
  ...
  - 当前步骤是否不需要新增用户决策
  - 实现后是否必须进入完整审查链
  - diff_review_target 是否已声明，或可由 docs/workflow/CURRENT_TASK.md 回滚点 / checkpoint
    记录明确推导
stop_conditions:
  - 当前步骤缺失或不清楚
...
verification:
  - 已按当前步骤执行
  - 已声明 review / verify / regression 链路使用的 diff_review_target
...
- 进入 `/review-diff` 前必须明确 `diff_review_target`；长任务 checkpoint 后应使用 `git diff <task-base>..HEAD` 或 `git diff <last-reviewed-checkpoint>..HEAD`。
