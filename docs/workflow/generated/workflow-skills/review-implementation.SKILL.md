...
handoff:
  ...
  failure: ask-user
conditional_handoff:
  mechanical_findings: implement-current-step
...
### finding_handoff_policy
- mechanical implementation findings within Allowed Files hand off to implement-current-step
- user_challenge findings hand off to ask-user
- contract or architecture changes hand off to ask-user
- scope widening required hand off to lock-scope
...
- 当前 Allowed Files 内的 mechanical implementation finding，可以 hand off 到 `/implement-current-step`。
...
3. Inspect the current diff and the implementation files affected by the diff.
4. Check `must_check` items before acting.
5. Respect `forbidden_writes` and current task boundaries.
6. If findings exist, classify them by `conditional_handoff` before using `handoff.failure`.
7. Mechanical implementation findings within Allowed Files hand off to `/implement-current-step`.
8. User challenge, contract, architecture, or product behavior findings hand off to `/ask-user`.
9. Scope widening findings hand off to `/lock-scope`.
10. If a stop condition does not match a `conditional_handoff` category, stop and hand off to `handoff.failure`.
11. Produce the artifact(s) described in `output`.
12. Hand off to `handoff.success` when the skill completes normally.
