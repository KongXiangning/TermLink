...
must_check:
  ...
  - 当前范围是否允许最小修复路径
  - 修复后是否复验原始失败场景
  - 修复后进入 review / verify / regression 链路前，diff_review_target 是否已声明或可由
...
- 修复后进入 `/review-diff` 前必须明确 `diff_review_target`；checkpoint 后应使用记录的 task base / checkpoint range。
