...
- 若 `docs/workflow/DECISIONS.md` 或 `.workflow-system/PROJECT_PROFILE.yaml` 已确认保留现有项目文档路径，则继续使用 `ARCHITECTURE.md`、`DATABASE.md`、`docs/architecture/**` 等当前路径；`docs/designs/**` 与 `docs/adoption/*inventory.md` 主要描述 canonical output bucket，而不是强制迁移命令。
...
| `ARCHITECTURE.md` / `DATABASE.md` / `docs/architecture/**` | 当前仓库保留的项目架构、数据与现状文档 | 需要读取项目事实，且 `docs/workflow/DECISIONS.md` 已确认这些文档保持原位时 |
...
| 对齐已有 workflow 资产 | `/realign-workflow-assets` | 当前 `.workflow-system/PROJECT_PROFILE.yaml`、已有 `docs/workflow/**` / `docs/designs/**` / `docs/adoption/**`、legacy root docs、runtime skill 目录、`AGENTS.md` / `CLAUDE.md` | 整理后的 workflow 文档目录、同步后的 runtime skills、更新后的 `AGENTS.md` / `CLAUDE.md` / `.workflow-system/PROJECT_PROFILE.yaml`；如 `docs/workflow/DECISIONS.md` 已确认保留现有项目文档路径，则按确认结果保留，不做静默迁移 |
...
| 老项目事实盘点 | `/legacy-inventory` | 由 profile / 仓库事实确定的代码目录、README/docs、package scripts、数据库和部署线索 | canonical 输出是 `docs/adoption/architecture-inventory.md`、`docs/adoption/database-inventory.md` 与 `docs/adoption/**`；若仓库已确认保留现有项目文档路径，则继续以当前项目文档为事实源并在 adoption 产物中记录 |
| 接管老项目治理基线 | `/adopt-existing-project` | `legacy-inventory` 产物、现有仓库事实、用户确认信息 | 首版 `.workflow-system/PROJECT_PROFILE.yaml`、`AGENTS.md`、`CLAUDE.md`、`docs/workflow/ROADMAP.md`、`docs/workflow/CONTRACTS.md`、`docs/workflow/BASELINES.md`、`docs/workflow/STATUS.md`、`docs/workflow/DECISIONS.md`，必要时更新 `docs/adoption/ADOPTION_REPORT.md`；不静默覆盖已确认保留的项目文档路径 |
