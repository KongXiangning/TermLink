...
### 归属 `20260615-001`（服务端 IPC 集成遗漏，通过 scope widening 吸收到本任务）

| ID | Severity | Status | Fix |
|---|---|---|---|
| **B1** | critical | ✅ resolved | `server.js`: `new CodexIpcFeed()` + `await feed.start()` before `server.listen()` |
| **B2** | critical | ✅ resolved | 新建 `codexIpcTransport.js` |
| **B3** | critical | ✅ resolved | Client: `'data'` → `'message'` event |
| **B4** | major | ✅ resolved | `terminalGateway.js`: `codex_ipc_conversations` 消息 |

### 归属 `20260615-002`（客户端缺陷）

| ID | Severity | Status | Fix |
|---|---|---|---|
| **B5** | major | ✅ resolved | `handleMessage` 新增 `case 'error'` |
| **B6** | critical | ✅ resolved | `/api/ws-ticket` flow |
| **B7** | minor | ✅ resolved | `_statusTimer` timeout 清理 |
| **B8** | minor | ✅ resolved | B4 解决 |
...
- 2026-06-15：执行 scope widening（用户选择方案 A）。已将 5 个 `src/**`/`tests/**` 文件正式纳入 Allowed Files。Safety mode 从 `normal` → `frozen-scope`。当前状态 `scope_widened_bugs_documented_ready_for_review`。
- 2026-06-15：修复 B5（`error` message 未处理：`handleMessage` 新增 `case 'error'`）和 B7（`showFollowerStatus` timeout 清理：`_statusTimer` 追踪）。当前状态 `bugs_fixed_b5_b7`。
- 2026-06-15：修复渲染偏差（TECHNICAL_DETAILS 偏差 1/2）：`renderSurface` 从 append-only 改为全量重绘，移除 `state.seenKeys`。当前状态 `rendering_fixed_full_redraw`。
- 2026-06-15：修复 userMessage IDE 上下文包装问题：`codexIpcThreadStream.js` 新增 `extractUserQuery()`，检测 `## My request for Codex:` 标记并只提取实际 query。端到端验证 PASS。
- 2026-06-16：完成 `/sync-current-task`。最终状态摘要：
  - **产出 11 files** (+2129/-627)：5 new (codex_ipc.html/js/css + Transport + TECH_DETAILS) + 6 modified (CURRENT_TASK + server.js + client + threadStream + gateway + test)
  - **8 bugs resolved**：B1-B4 (服务端遗漏, scope widening 吸收)、B5 (error 处理)、B6 (ticket)、B7 (timeout)、B8 (conversation list)
  - **验收**：端到端 IPC online 验证通过（4 conversations, 87 items），no-IPC fallback 真机通过。189 pass/0 fail 自动化回归。
  - **已知 gap**: `codexIpcThreadStream.js` 未正式列入 Allowed Files（review-diff F1 minor）
  - **Commit**: `9603ea7` on `ipc-tun`
  - 当前状态 `all_implementation_completed_ready_for_closeout`，下一步 `/sync-status` → `/close-current-task`
