---
name: resume-paused-task
preamble-tier: 2
version: 0.1.0
description: >
  Restore docs/workflow/CURRENT_TASK.md from one explicit paused package and
  return the flow to review-current-task.
purpose: >
  从一个明确的 paused suspended package 恢复 live docs/workflow/CURRENT_TASK.md，并把流程固定交回
  review-current-task。
stage: 阶段 7：状态同步
trigger: |
  用户明确要求恢复一个 paused task，且目标 package 已显式给定或可无歧义解析时。
inputs:
  - current_task
  - paused_package_target
...
stop_conditions:
  - 目标 package 缺失、歧义，或试图“自动挑最新 paused package”
  - "`artifact_kind` 不是 `paused`"
...
output:
  ...
  - rehydrated paused package
  - resume gate alignment record
handoff:
  success: review-current-task
  failure: ask-user
decision_policy:
  mechanical: 可以自动整理显式 package 解析、payload restore 与 gate 对齐顺序。
  taste: 不得把“恢复哪个 package”静默替用户猜测。
  user_challenge: 不得绕过 ready-for-resume marker、完整 payload 或 review gate。
verification:
  - "`writes` 只覆盖 `docs/workflow/CURRENT_TASK.md` 与 `TASKS/paused/**`"
  - 只接受显式、无歧义、`ready_for_resume + recovery_only` 的 paused package 输入
...
notes:
  - 不允许“自动挑最新一个 paused package”这类模糊恢复策略。
rehydration_markers:
  - ready_for_resume
  - recovery_only
  - rehydrated
...
# Skill: resume-paused-task

## Purpose

从一个明确的 paused suspended package 恢复 live `docs/workflow/CURRENT_TASK.md`，并把流程固定交回 `review-current-task`。
