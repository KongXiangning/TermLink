...
stage: 初始化
trigger: |
  当老项目已存在，但还没有 workflow 治理工件，或者准备正式接入 workflow 时。
inputs:
  - existing_repository
  - user_request
  - business_context
...
forbidden_writes:
  ...
  - scripts/**
must_check:
  - 现有仓库中哪些是已证实事实，哪些只是推断
  - legacy-inventory 输出中的 stable / fragile / unknown / deprecated 是否清晰
  - 现有命令、目录结构、契约边界和工作流是否已被扫描
  - 需要用户确认的业务背景、命名约定和边界是否显式列出
  - 首版治理基线是否只固化了已确认事实
stop_conditions:
  - 核心仓库事实无法从代码或文档中得到证据
  - 关键业务背景缺失且影响治理基线表述
  - 发现多个互相冲突的事实来源但无法判定真值
output:
  - 首版 .workflow-system/PROJECT_PROFILE.yaml
  - 首版 host 指引文件（同时生成 `AGENTS.md` / `CLAUDE.md`）
  - 首版 docs/workflow/ROADMAP.md / docs/workflow/CONTRACTS.md / docs/workflow/BASELINES.md / docs/workflow/STATUS.md / docs/workflow/DECISIONS.md
  - 必要时更新 `docs/adoption/ADOPTION_REPORT.md`
handoff:
  success: create-current-task
  failure: ask-user
decision_policy:
  mechanical: 可以整理扫描结果、抽取命令、汇总目录和契约事实。
  taste: 对协作方式、文档语气、治理粒度不要擅自定稿，必须标出需要确认的点。
  user_challenge: 不得把推断当作事实写入治理基线；必须先抽取、再确认、再落地。
verification:
...
allowed-tools:
  ...
  - AskUserQuestion
notes:
  - 这是老项目接管 skill，不改业务代码，只建立治理基线。
  - 必须优先消费 legacy-inventory 的事实盘点，再向用户确认不能从仓库直接证明的部分。
  - 完成后下一步通常是 create-current-task。
---

# Skill: adopt-existing-project
...
1. 先读取 `.workflow-system/WORKFLOW_PROTOCOL.md`、`.workflow-system/FILE_SCHEMAS.md`、`templates/docs/`，再读取 `legacy-inventory` 产物（如 `ARCHITECTURE.md`、`DATABASE.md`、`docs/adoption/`、`docs/workflow/ROADMAP.md`、`docs/workflow/BASELINES.md`）和仓库中的现有事实：代码结构、目录布局、README/docs、`package.json` scripts、测试入口、部署/运行线索、已有协作文档。
2. 把扫描结果拆成两类：  
   - 可被仓库直接证明的事实  
   - 必须由用户补充或确认的事实
...
- 不修改 `src/`、`test/`、`scripts/` 下的业务或执行逻辑。
- 不把猜测、默认值、或单次偶然现象写成项目基线。
- 不把 inferred 或 unknown 写成 confirmed。
- 不跳过用户确认直接冻结高影响结论。
